...
namespace OnlineSignitureVerification.InPut
{
    ...
    static class readSample
    {
        ...
        public static double sampling = 5;
        ...
        public static List<List<double>> GetDataFromFile(int user, int SignId, bool reshape = true)
        {
            if (!reshape)
            {
                //TODO: reshape nélkülüli beolvasás
            ...
            else
            {
                ...
                string resPath = @"C:\repos\2018TavaszOnlab\ConsoleApp3\resources\"; //Program.ResPath;

                switch (resFileName)
                {
                    case ResFileName.sample:
                        path = String.Format("{0}USER{1}_{2}.txt", resPath, user, SignId);
                        break;
                    case ResFileName.task1:
                        path = String.Format("{0}U{1}S{2}.txt", resPath + "task1\\", user, SignId);
                        break;
                    case ResFileName.task2:
                        path = String.Format("{0}U{1}S{2}.txt", resPath + "task2\\", user, SignId);
                        break;
                    default:
